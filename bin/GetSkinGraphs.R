...
option_list <- list(
  ...
  make_option(c("-o", "--output"),
    ...
    metavar = "character"),
  make_option(c("-a", "--allconnections"),
    action="store_true",
    default=FALSE,
    help = "All connections should be reported. [default %default]"),
  make_option(c("-p", "--patient"),
    type = "character",
    default = NULL,
    help = "Patient ID to use (this speeds up run time).",
    metavar = "character")
)
...
if(opt$allconnections) {
    write("Getting all connections.", stderr())
    write(paste("Pulling graph for", opt$location, opt$timepoint, opt$patient, sep = " - "), stderr())
    sampleidquery <- paste("
    MATCH
        (x:SRP049645)-->(y)-[d]->(z:Phage),
        (a:Bacterial_Host)<-[e]-(b),
        (b)<--(i:", opt$patient, ")-->(y),
        (b)<--(t:", opt$timepoint, ")-->(y),
        (b)<--(k:", opt$location, ")-->(y)
    ...
    write("Getting infectious connections.", stderr())
    sampleidquery <- paste("
    MATCH
