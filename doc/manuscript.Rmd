...
The longest contig from each OGU was used as its representative sequence for identification. Representative sequences were aligned to the European Nucleotide Archive (ENA) bacterial reference database using the blastn algorithm (e-value < $10^{-5}$). 79% of the bacterial OGUs were able to be identified. <!--what was this ID used for? i don't recall any discussion of inferred cluster taxonomic identity-->

## Phage OGU Identification
To confirm a lack of phage sequences in the bacterial OGU dataset, we performed a relatively strict <!--indeed, this is relative. can you relate it? What is the distribution of evalues or bitscores from this blastp? any anecdotes that it was a useful cut-off?--> blast nucleotide alignment of the bacterial OGU representative sequences using an e-value < $10^{-25}$. We used a stricter threshold because we know there are genomic similarities between bacteria and phage OGUs from the interactive model, but we were interested in contigs with high enough similarity to references that they may indeed be from phages. 2% of the OGUs had nucleotide similarities to known bacteriophage genomes, although the alignments were short (a couple of kb) and represented a small fraction of the alignment sequences. <!-- shouldn't you also consider a relative length component? Meaning, contigs with only phage similarity along its entire length are more likely phage than contigs with only small portion similar to phage, while rest has similarity to microbes...thus more likley prophage and ok to stay classified as bacteria. Why not just run already created probabilitic models that use many criteria, like VirSorter? Then you get predictions of phage (cat 1-3) versus propahge (cat 4-6) and the confidence of the prediction. -->
