...
In addition to diet, there was also an association between obesity and network structure **(Figure \ref{dietnetworks} C-D)**. The obesity-associated network demonstrated a higher degree centrality **(Figure \ref{dietnetworks} C)** but a less closeness centrality, compared to the healthy controls **(Figure \ref{dietnetworks} D)**. This meant that the obesity network was overall less connected, having microbes further from all other microbes within the community.
