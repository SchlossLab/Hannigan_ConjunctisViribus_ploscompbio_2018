...
The bacterial and viral sequences from these studies were quality filtered and assembled into contigs. We generated clusters of related bacterial and phage contigs based on their k-mer frequencies and co-abundance patterns using the CONCOCT algorithm **(Supplemental Figure \ref{ContigStats} - \ref{ClusterStats})**. Contig lengths within clusters ranged between $10^{3}$ and $10^{5.5}$, confirming the majority of contigs represented genome fragments, rather than complete genomes  **(Supplemental Figure \ref{ContigStats} - \ref{ClusterStats})**. We were also able to identify the majority (79%) of bacteria clusters, agreeing with previous reports [@Minot:2011ez, @Hannigan:2015fz, @Turnbaugh:2009ei]. We interpreted these clusters as operational units of related bacteria and phage genomes, which we defined as operational genomic units (OGUs). These OGUs are conceptually similar to the operational taxonomic unit (OTU) and operational protein family (OPF) definitions used for grouping highly similar 16S rRNA gene and open reading frame sequences, respectively [@Schloss:2008ht].
...
In addition to diet, we also observed a potential association between obesity and network structure **(Figure \ref{dietnetworks} C-D)**. The obesity-associated network demonstrated a higher degree centrality **(Figure \ref{dietnetworks} C)**, but less closeness centrality compared to the healthy controls **(Figure \ref{dietnetworks} D)**. These results suggest that the obesity network may have been overall less connected, having microbes further from all other microbes within the community.
...
## Bacterial OGU Identification
The longest contig from each OGU was used as its representative sequence for identification. Represetnative sequences were aligned to the European Nucleotide Archive (ENA) bacterial reference database using the blastn algorithm (e-value < $10^{-5}$).
